...
- **Open:** `1 open` — Blockers: none; Majors: `F1` (Minors/Nits: see §3)
...
| F9 | Nit | `src/aspark_guard/activity.py:107` | Each file can reach 2 MB − 1 + one line, so the pair can hold 4 MB + 2 lines, not AC-4.3's "4 MB plus one line"; `test_activity_rotation.py:80` hides this in 1 KB slack. **Fix:** rotate when `size + len(line) > MAX_BYTES`, or amend the wording. | open |
| F10 | Nit | `src/aspark_guard/activity.py:194-196` | Starts with empty `agent_type` are skipped too; D-T1-4 documents only stops. Consistent and harmless. **Fix:** extend D-T1-4's text. | open |
| F11 | Nit | `src/aspark_guard/activity.py:310-329` | Pairing reads without the lock; a rotation between reading `.1` and the current file can hide the start → `duration_ms: null` (safe direction). **Fix:** accept and note, or read under the lock. | open |
| F12 | Nit | `src/aspark_guard/activity.py:170-174` | A hook killed between the `O_EXCL` create and the write leaves an empty `.gitignore` that is never repaired, and activity files then show in `git status`. **Fix:** treat an empty file as absent (rewrite via tmp + `os.replace` only when size 0). | open |
