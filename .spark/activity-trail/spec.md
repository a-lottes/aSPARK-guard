...
**Handoff**
- **Status:** `approved` — approved by the user at the spec gate, 2026-09-22. Next ceremony step: `/sprint-plan`, starting with the A3 hook spike.
- **Summary:** aspark-guard keeps a local, metadata-only log of live activity (on by default where `.spark/` exists, capped at 2 MB + 1 rotated file, kept out of git). It records whether each session is busy, idle, waiting or ended, and each subagent's start, finish, duration and short task label, so a read-only cockpit can show what the team is doing right now. Only facts are recorded, never inferred ones. Tool-level and phase events are Won't this cycle.
- **Open:** `none`. A3 was accepted as a risk by the user; a /sprint-plan spike settles it before any build task.
...
- **Success signal:**
  1. *Completeness:* during the next full loop in `aspark-vscode` with the guard installed, every subagent run the Claude UI shows has exactly one start and one finish line in the log. Each duration is within ±1 s of the run's duration in the session transcript (a one-off manual check by the author; no consumer reads transcripts).
...
| A4 | One `agent_id` can run more than once: resumed agents. Real data has `a44343ff…` finishing 5 times. A run is one start→finish pair, not one agent. | assumed from evidence |
...
- [ ] AC-2.2: Given a recorded `subagent_start`, when that subagent finishes, then the log gains one `agent_run` line with the same `agent_id`, `agent_type` and `feature`, and a `duration_ms` equal to the time between the two events (±50 ms).
- [ ] AC-2.3: Given an `agent_id` that runs a second time (resumed agent), when that run finishes, then its `duration_ms` covers only the second run, measured from the latest unmatched start of that `agent_id` in that session.
- [ ] AC-2.4: Given a subagent whose start was not recorded (guard installed mid-run, start hook failed, start rotated out), when it finishes, then the `agent_run` line is still written, with `duration_ms: null`. No duration is guessed.
...
- [ ] AC-2.7: Given this feature is built, when a subagent finishes, then `trail.jsonl` still gains exactly the line it gains today, byte-format unchanged.
...
No constitution, so no lenses apply. Privacy, log size and hook latency are the user's named concerns.

| # | Category | Requirement (measurable) | How it's verified |
|---|---|---|---|
| NFR-1 | Performance | Each activity hook invocation: p95 ≤ 100 ms over 50 runs on the author's M-series Mac, both with and without `.spark/`, and with the log at its 2 MB cap. Max ≤ 500 ms in the same run, far from the 5 s manifest timeout. Ledger and gate hooks get no slower than today's ~50 ms (±10 ms). Ordinary tool calls get no new hook (AC-4.8). The README's cost section states the measured cost this feature adds per prompt and per subagent in projects without `.spark/`. | benchmark recorded in `docs/evidence.md` · /peer-review |
...
| NFR-6 | Compatibility | The existing tests still pass unchanged. Ledger and trail line formats are unchanged. Hook payload fixtures are added for every newly used event, in the shape the harness actually sends (A3). | automated · /peer-review |
...
- Changing `trail.jsonl`: no duration, start or stop_reason fix there. It stays the committed run counter.
...
| C2 | 2026-09-22 | Orphaned starts, crash between start and stop, resumed agents? | AC-1.5, AC-2.3, AC-2.4, AC-2.6. A run pairs the latest unmatched start per `agent_id`; a missing start gives a `null` duration. Kill handling settled in C11. |
...
- [x] Clarify pass done: no ambiguity left unresolved or unparked (C1–C13)
- [x] Open questions are resolved or explicitly accepted as risk (A3 accepted by the user, C13)
- [x] Out-of-scope section is filled (something was consciously cut)
- [x] Constitution (`.spark/constitution.md`) respected, or conflicts recorded as open questions (none exists; the README invariants were treated as binding: NFR-7, AC-4.1, AC-3.3)
...
- [x] Line budget respected: Ist ~220 / Soll ~250 (excluding HTML comments)
